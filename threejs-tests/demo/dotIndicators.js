...
export function setupBranchDotIndicators(scene, camera, tree, meshToDotMap, highlightBranch, unhighlightBranch) {
  ...
  const dotPositions = [];
  let dotsVisible = true; // Track visibility state
  ...
  function addBottomDots(treeRoot) {
    ...
    treeRoot.traverse((obj) => {
      if (obj.isMesh && obj.geometry && obj !== treeRoot) {
        ...
        dotElement.style.backgroundColor = "#00ff33";
        dotElement.style.borderRadius = "50%";
        dotElement.style.boxShadow = "0 0 4px rgba(0, 255, 51, 0.8)";
  ...
  return { updateDotPositions, removeMeshDots, toggleDotsVisibility };
}
