...
export class SimpleTree extends THREE.Group {
  ...
  generate() {
    ...
    this.createBranch(this, trunkLevel, trunkHeight, trunkRadius, 0); // the 0 is to place it at the bottom of the scene
  }

  createBranch(parent, level, height, radius, parentHeight = 0) {
    ...
    if (nextLevel <= this.maxLevels) {
      ...
      for (let i = 0; i < numBranches; i++) {
        ...
        // Call recursion
        const childMesh = this.createBranch(
          mesh,
          nextLevel,
          childHeight,
          childRadius,
